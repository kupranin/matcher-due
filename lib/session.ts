 ...
 * Use getSessionTokenFromRequest(request) in route handlers so auth is read from the incoming request only.
 */
...
/** Read session token from the request (Authorization: Bearer or Cookie). Request-only, no cookies() call. */
